...
public class DeepFryerCommand implements IServerCommand {
	...
	public void execute(Message msg, TextChannel channel, Guild g, Member sender, String content, String[] args) {
		if (deepfryers.contains(sender.getId())) {
		...
		Runnable r = new Runnable() {
			...
			public void run() {
				try {
					...
					if (!isImage) {
						deepfryers.remove(sender.getId());
					...
					if (original == null) {
						deepfryers.remove(sender.getId());
						sendMessage(channel, sender.getAsMention() + " i was unable to process the image.");
						return;
					}
					...
					if (!force) {
						if (f2.exists()) {
							Logger.log("User: " + sender.getUser().getName() + " supplied an existing deep fried meme.");
							deepfryers.remove(sender.getId());
							sendFile(channel, f2, sender.getAsMention());
							return;
						}
					} else {
						Logger.log("User: " + sender.getUser().getName() + " forced re-deep-fried meme: " + url.toString());
					}

					int size = image.length / (1024 * 1024);

					if (size > 2 || original.getWidth() > 3500 || original.getHeight() > 3500) {
						if (!canExecuteABigImage(sender)) {
							sendMessage(channel, sender.getAsMention() + " BOI YOUR FILE IS 2 POWERFUL!");
							deepfryers.remove(sender.getAsMention());
							return;
						} else {
							sendMessage(channel, sender.getAsMention() + " your file is a little big, might take a while for it to process.");
						}
					}

					Logger.log("Deep frying meme from: " + sender.getUser().getName() + " url: " + url.toString());
		...
		t.setName("DeepFryer: " + sender.getUser().getName());
		t.start();
	}
	...
	public boolean canExecuteABigImage(Member user) {
